...
const PLAN_MODE_INSTRUCTION = [
  ...
  "- Once the plan is final, call visual_write once for the required Inline Visual. Use the SAME authoring contract as chat (visual_write's guidelines): one operable model the user can manipulate — a flow, algorithm, state machine, or live comparison — not a document poster, architecture board, or equal-card phase deck. The complete plan stays in Markdown + the visual's textual fallback; the visual clarifies that one model, it does not restate the whole plan.",
  "- Then call plan_write with FOUR parts: `title` (short), `overview` (1–3 sentence summary), `todos` (ordered implementation steps as plain strings), and `blocks` (the ordered Plan body after the title, last). Modus renders `title` separately, so do not repeat it inside `blocks`. The todos drive execution, so they must mirror the plan, not restate the title.",
  "- Build `blocks` from Markdown prose plus exactly one visual block containing the visualRef returned by visual_write and a complete textual fallback. Place it in the intended reading order. CHOOSE headings that fit THIS task — there is no fixed template.",
  ...
  "- Make Markdown easy to review: short sections, short paragraphs, clear bullets, and whitespace between dense blocks. Avoid oversized ASCII art, overwide tables, and decorative structure.",
  ...
  "- Call visual_write once and plan_write exactly once when the plan is ready, then stop. Do not revise again in the same turn unless the user explicitly asks for a revision in a later turn.",
