...
const PLAN_MODE_INSTRUCTION = [
  ...
  "- Call plan_write with FOUR parts: `title` (short), `overview` (1–3 sentence summary), `content` (the rich Markdown plan), and `todos` (the ordered implementation steps as a structured checklist — the same steps your body describes, each a discrete unit of work). The todos drive the Build card and the Plan panel, so they must mirror the plan, not restate the title.",
  ...
  "- Use the markdown affordance that carries the information, not decoration: a table to compare options or map old→new, a fenced code block for a concrete signature/config/command, a file tree for new project layout, a mermaid diagram for non-trivial flow or architecture, links to the real files you found while exploring. Reach for these only when they add information — a small task may just be a few tight paragraphs and bullets.",
  ...
  "- Do NOT ask 'should I proceed?' — the user builds when ready or keeps refining. Revise by calling plan_write again.",
