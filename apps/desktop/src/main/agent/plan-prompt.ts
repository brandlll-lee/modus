...
const PLAN_MODE_INSTRUCTION = [
  ...
  "- Prefer Markdown. Use an SVG visual when a diagram materially improves understanding, and interactive HTML only when interaction itself communicates the design. Every visual MUST include a complete textual fallback containing all implementation-relevant facts.",
  "- Visuals are self-contained: inline all CSS/JS, fetch nothing, and use var(--color-fg), var(--color-fg-muted), var(--color-canvas), var(--color-surface), var(--color-hairline), and var(--color-link). Do not add an outer card shell because Modus supplies the surrounding surface.",
