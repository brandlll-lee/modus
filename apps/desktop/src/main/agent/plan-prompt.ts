...
const PLAN_MODE_INSTRUCTION = [
  ...
  "- Call plan_write with a focused Markdown plan. CHOOSE headings that fit THIS task — there is no fixed template. A bugfix, a refactor, a new feature, and a research/product plan each want a different shape; do not force them into the same sections.",
  ...
  "- Use the markdown affordance that carries the information, not decoration: a table to compare options, a fenced code block for a concrete signature/config/command, a mermaid diagram for non-trivial flow or architecture, links to the real files you found while exploring. Reach for these only when they add information — a small task may just be a few tight paragraphs and bullets.",
