...
const PLAN_MODE_INSTRUCTION = [
  ...
  "- Call plan_write with FOUR parts: `title` (short), `overview` (1–3 sentence summary), `todos` (ordered implementation steps as plain strings), and `blocks` (the ordered Plan body, last). The todos drive execution, so they must mirror the plan, not restate the title.",
  "- Build `blocks` from Markdown prose and optional Inline Visuals in reading order. CHOOSE headings that fit THIS task — there is no fixed template.",
  "- Prefer Markdown. Add a visual only when it materially improves review; usually one primary visual is enough. Use SVG for static relationships and HTML only when interaction itself communicates the design.",
  "- Visual copy and fallback follow the language of the surrounding plan and current user request, or an explicitly requested language. Preserve paths, APIs, code identifiers, commands, and product names verbatim.",
  "- Every visual MUST include a complete textual fallback containing all implementation-relevant facts. The plan_write tool defines the authoritative visual quality and safety contract.",
